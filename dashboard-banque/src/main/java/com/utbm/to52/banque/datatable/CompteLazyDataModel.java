 ...
 * @author kommit
 */
public class CompteLazyDataModel extends LazyDataModel<Compte>{
    private static final long serialVersionUID = -6123945723069023025L;
	private final transient ICompteMetier service;
	...
	public CompteLazyDataModel(ICompteMetier service) {
		this.service = service;
	}
	
	@Override
	public Object getRowKey(Compte Compte) {
		return Compte.getCodeCompte();
	}
	
	@Override
	public Compte getRowData(String rowKey) {
		Long rowId = Long.valueOf(rowKey);
		List<Compte> operations = (List<Compte>) super.getWrappedData();
		return operations.stream().filter(Compte -> Compte.getCodeCompte().equals(rowId)).findAny().orElse(null);
	}
	...
	@Override
	public List<Compte> load(int first, int pageSize, String sortField, SortOrder sortOrder,
	...
	private List<Compte> filterAndSort(int first, int pageSize, Map<String, Object> filters, Sort sort) {
		Map<String, String> filtersMap = filters.entrySet().stream()
				.collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().toString()));
		          Page<Compte> page = service.findByFilter(filtersMap, new PageRequest(first / pageSize, pageSize, sort));
